...
import abc
import itertools
from typing import Any, Callable, Generic, Iterator, Optional, TypeGuard, TypeVar, overload
...
class _Text_Lines(Generic[T], metaclass=abc.ABCMeta):
    ...
    def _is_list_T(self, text: Any) -> TypeGuard[list[T]]:
        return isinstance(text, list) and (len(text) == 0 or isinstance(text[0], Text_Line))
...
class Text_Lines(_Text_Lines[Text_Line]):
    ...
    def _to_list_T(self, text: str | list[str]) -> list[Text_Line]:
        ...
        return [Text_Line(s, i) for i, s in enumerate(texts)]

    def get_instance(self, texts: list[Text_Line]) -> Self:
        return Text_Lines(texts)

    def change_idx(self, fn: Callable[[Text_Line], int]) -> Self:
        return self.get_instance(
            [
                Text_Line(text=line.text, idx=fn(line), sep=line.sep, depth=line.depth, has_child=line.has_child)
                for line in self
            ]
        )
...
class Texts_Printer:
    ...
    Lines = TypeVar("Lines", bound=Text_Lines)
